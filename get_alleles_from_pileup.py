...
def write_alleles(sample_name, positions, array):
	...
	print 'Positions covered: %i (%.2f %%)' %(len(frag) - frag.count('-'), 100 - (float(frag.count('-')) / len(positions) * 100))
	print 'Gaps: %i (%.2f %%)' %(frag.count('-'), float(frag.count('-')) / len(positions) * 100)
	print 'Mean chastity: %.2f %%' %(mean_chastity)
	if mean_chastity < 90:
		print 'CHASTITY WARNING: low chastity can severely affect accuracy of placement'

	id = sample_name
	with open('alleles/%s_cov%i.fasta' %(sample_name, cov), 'w') as file_out:
		print >>file_out, '>%s_cov%i\n%s\n' %(id, cov, frag),
